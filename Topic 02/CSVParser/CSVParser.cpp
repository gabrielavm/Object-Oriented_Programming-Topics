...
size_t parseRow(const char* row, Row& toReturn)
{
	...
	while (!ss.eof())
	{
		ss.getline(toReturn[currentColumnCount++], GlobalConstants::FIELD_MAX_SIZE, GlobalConstants::SEPАРАТОР);
	}
...
bool modifyFieldInColumn(CsvTable& csv, const char* columnName, const char* newValues, char sep)
{
